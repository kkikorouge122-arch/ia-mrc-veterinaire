...
def generer_pdf_bio(prediction_bio, confidence_bio, recommandations_bio, age, pression, densite, creatinine, uree, hemoglobine):
    from fpdf import FPDF
    import io
    ...
    pdf.cell(190, 10, nettoyer_texte("Rapport d'Analyse Biologique Vétérinaire"), 0, 1, "C")
    pdf.ln(5)

    # Section Informations / Constantes
    pdf.set_font("Arial", style="B", size=12)
    pdf.cell(190, 8, nettoyer_texte("Constantes de l'animal :"), 0, 1)
    pdf.set_font("Arial", size=11)

    # Liste des paramètres à afficher
    parametres = [
        ("Âge", f"{age} ans"),
        ("Pression Artérielle", f"{pression} mmHg"),
        ("Densité Urinaire", f"{densite}"),
        ("Créatinine", f"{creatinine} mg/L"),
        ("Urée", f"{uree} g/L"),
        ("Hémoglobine", f"{hemoglobine} g/dL")
    ]

    # Affichage en tableau simple sans new_x/new_y (le 1 en fin de ligne gère le retour à la ligne)
    for nom, valeur in parametres:
        pdf.cell(90, 7, nettoyer_texte(f"  {nom} :"), 0, 0)
        pdf.cell(100, 7, nettoyer_texte(valeur), 0, 1)

    pdf.ln(5)

    # Section Résultats
    pdf.set_font("Arial", style="B", size=12)
    pdf.cell(190, 8, nettoyer_texte("Résultat du diagnostic IA :"), 0, 1)
    pdf.set_font("Arial", size=11)

    statut = "Anomalie Détectée" if prediction_bio == 1 else "Normal"
    pdf.cell(90, 7, nettoyer_texte("  Statut :"), 0, 0)
    pdf.cell(100, 7, nettoyer_texte(statut), 0, 1)
    pdf.cell(90, 7, nettoyer_texte("  Confiance :"), 0, 0)
    pdf.cell(100, 7, nettoyer_texte(f"{confidence_bio * 100:.2f}%"), 0, 1)

    pdf.ln(5)

    # Section Recommandations
    pdf.set_font("Arial", style="B", size=12)
    pdf.cell(190, 8, nettoyer_texte("Recommandations cliniques :"), 0, 1)
    pdf.set_font("Arial", size=11)
    
    # Écriture des recommandations (multi-lignes sécurisé)
    pdf.multi_cell(190, 6, nettoyer_texte(recommandations_bio))

        # Génération du contenu du PDF
    pdf_output = pdf.output()
    
    # Conversion stricte en octets pour Streamlit
    if isinstance(pdf_output, str):
        return bytes(pdf_output, encoding="latin-1")
